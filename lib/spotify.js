 ...
 function createSpotifyClient({
   ...
   async function searchTracksByYear(year, { limit = 50 } = {}) {
     const y = clampInt(year, { min: 1900, max: 3000, fallback: year });
     const q = encodeURIComponent(`year:${y}`);
     const l = clampInt(limit, { min: 1, max: 50, fallback: 50 });
     const json = await apiGet(`/search?q=${q}&type=track&limit=${l}`);
