...
// Mulberry32 — fast, good-quality 32-bit PRNG.  Same seed → same sequence,
// so all serverless invocations on the same UTC day pick the same image.
...
function dailySeed() {
 ...
 function createSpotifyClient({
  ...
  async function searchTracksByYear(year, { limit = 10, market = 'GB' } = {}) {
    ...
    const m = encodeURIComponent((market ?? 'GB').toString());
 ...
 // ── Daily album-cover cache ───────────────────────────────────────────────────
 // Same pattern as the city image cache: one result per UTC day, busted at
 // 00:00 UTC via a self-scheduling setTimeout.  The object is mutated in-place
 // (never reassigned) so exported references stay valid.
 const spotifyImageCache = { result: null, date: null };

 async function getDailyAlbumCover({ client, rng } = {}) {
   if (!client) throw new Error('getDailyAlbumCover requires a spotify client');

   const now = new Date();
   const today = `${now.getUTCFullYear()}-${String(now.getUTCMonth() + 1).padStart(2, '0')}-${String(now.getUTCDate()).padStart(2, '0')}`;

   if (spotifyImageCache.date === today && spotifyImageCache.result) {
     return spotifyImageCache.result;
   }

   const dailyRng = rng ?? seededRng(dailySeed());
   const year = randomYear({ rng: dailyRng });
   const result = await getRandomAlbumCoverFromYear({ client, year, rng: dailyRng });

   spotifyImageCache.result = result;
   spotifyImageCache.date = today;

   // Schedule a cache bust at the next 00:00 UTC.
